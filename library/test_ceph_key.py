...
class TestCephKeyModule(object):
    ...
    def test_generate_ceph_cmd_list_container(self):
        ...
        result = ceph_key.generate_ceph_cmd(fake_cluster, fake_args, fake_containerized)
        assert result == expected_command_list
    ...
    def test_generate_ceph_authtool_cmd_non_container_no_auid(self):
        ...
        result = ceph_key.generate_ceph_authtool_cmd(fake_cluster, fake_name, fake_secret, fake_caps, fake_auid, fake_dest)
        assert result == expected_command_list
    ...
    def test_generate_ceph_authtool_cmd_container(self):
        ...
        result = ceph_key.generate_ceph_authtool_cmd(fake_cluster, fake_name, fake_secret, fake_caps, fake_auid, fake_dest, fake_containerized)
        assert result == expected_command_list
    ...
    def test_create_key_non_container(self):
        ...
        fake_file_destination = os.path.join(fake_dest + fake_cluster + "." + fake_name + ".keyring")
        expected_command_list = [
            ['ceph-authtool', '--create-keyring', fake_file_destination, '--name', fake_name, '--add-key', fake_secret, '--cap', 'mon', 'allow *', '--cap', 'osd', 'allow rwx'],
            ['ceph', '--cluster', fake_cluster, 'auth', 'import', '-i', fake_file_destination],
        ]
        result = ceph_key.create_key(fake_module, fake_result, fake_cluster, fake_name, fake_secret, fake_caps, fake_import_key, fake_auid, fake_dest)
        assert result == expected_command_list
    ...
    def test_create_key_container(self):
        ...
        fake_file_destination = os.path.join(fake_dest + fake_cluster + "." + fake_name + ".keyring")
        expected_command_list = [
            ['docker', 'exec', '-ti', 'ceph-mon', 'ceph-authtool', '--create-keyring', fake_file_destination, '--name', fake_name, '--add-key', fake_secret, '--cap', 'mon', 'allow *', '--cap', 'osd', 'allow rwx'],
            ['docker', 'exec', '-ti', 'ceph-mon', 'ceph', '--cluster', fake_cluster, 'auth', 'import', '-i', fake_file_destination],
        ]
        result = ceph_key.create_key(fake_module, fake_result, fake_cluster, fake_name, fake_secret, fake_caps, fake_import_key, fake_auid, fake_dest, fake_containerized)
        assert result == expected_command_list
    ...
    def test_update_key_non_container(self):
        ...
        expected_command_list = [
            ['ceph', '--cluster', fake_cluster, 'auth', 'caps', fake_name, 'mon', 'allow *', 'osd', 'allow rwx'],
        ]
    ...
    def test_update_key_container(self):
        ...
        expected_command_list = [
            ['docker', 'exec', '-ti', 'ceph-mon', 'ceph', '--cluster', fake_cluster, 'auth', 'caps', fake_name, 'mon', 'allow *', 'osd', 'allow rwx'],
        ]
        result = ceph_key.update_key(fake_cluster, fake_name, fake_caps, fake_containerized)
        assert result == expected_command_list
    ...
    def test_delete_key_container(self):
        ...
        fake_containerized = "docker exec -ti ceph-mon"
        expected_command_list = [
            ['docker', 'exec', '-ti', 'ceph-mon', 'ceph', '--cluster', fake_cluster, 'auth', 'del', fake_name],
        ]
        result = ceph_key.delete_key(fake_cluster, fake_name, fake_containerized)
        assert result == expected_command_list
    ...
    def test_info_key_non_container(self):
        ...
        fake_name = "client.fake"
        expected_command_list = [
            ['ceph', '--cluster', fake_cluster, 'auth', 'get', fake_name, '-f', 'json'],
        ]
    ...
    def test_info_key_container(self):
        ...
        fake_containerized = "docker exec -ti ceph-mon"
        expected_command_list = [
            ['docker', 'exec', '-ti', 'ceph-mon', 'ceph', '--cluster', fake_cluster, 'auth', 'get', fake_name, '-f', 'json'],
        ]
    ...
    def test_list_key_container(self):
        ...
        fake_containerized = "docker exec -ti ceph-mon"
        expected_command_list = [
            ['docker', 'exec', '-ti', 'ceph-mon', 'ceph', '--cluster', fake_cluster, 'auth', 'ls', '-f', 'json'],
        ]
